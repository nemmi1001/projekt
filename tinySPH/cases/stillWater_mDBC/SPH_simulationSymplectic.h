...
struct SPH_simulation
{
	...
	//	Density_to_pressure(particles);
		...
		/* Apply mDBC, compute pressure */
	//to já	mDBC_compute_density_mdbcGeo(particles, simulation_data);
		//Density_to_pressure(particles);
	...
	SPH_simulation
	()
	: particles(hh, kap, cs, 8960, visco, delta, dp, 384.00,383.),
			simulation_data(x_0, x_1, y_0, y_1)
	{ // kapa je prvni ocel 40, kapacita posledni ocel 450
