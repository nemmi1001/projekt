...
(Particle_system &particles, Simulation_data simulation_data, std::string filename, real x0, real y0, real xm, real ym)
{
	...
	*/
	...
	std::array<real, 5> interpolatedFields;
	...
	vel[POS(x, y, ncx, ncy)].y = interpolatedFields[3];
      	tem[POS(x, y, ncx, ncy)] = interpolatedFields[4];  
	...
	write_INTERPOLATED_mesh_to_ASCII_VTK(ncx, ncy, dh, x0, y0, vel, rho, press, tem, filename);
