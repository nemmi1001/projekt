...
// orientation +1: -1:
{
	...
	for(int l = 0; l < nvl; l++)
	{
		...
		for(int i = 0; i < npart+1; i++)
		{
			...
			real tt=0;
			//T(0)=1, T(l)=51
			tt=1+i*dp*(10000-1)/len;
			v.x = 0; v.y = 0;
			a.x = 0; a.y = 0;
			particles.Add_particle(wall, 0, particles.data_const.rho0, r, v, a, tt);
...
void Create_Wall_y_mdbc(Particle_system & particles, double x, double y_0, double y_1, int orientation, double x_w,real T)
// orientation +1: -1:
{
	real dp = particles.data_const.dp;
	real kh = particles.data_const.h * particles.data_const.kap;
	...
	for(int l = 0; l < nvl; l++)
	{
		...
		for(int i = 0; i < npart+1; i++)
		{
			...
			particles.Add_particle(wall, 0, particles.data_const.rho0, r, v, a, T);
...
void Create_corner_mdbc(Particle_system & particles, int orientationX, int orientationY, realvec xyv,real T)
// orientation +1: -1:
{
	real dp = particles.data_const.dp;
	real kh = particles.data_const.h * particles.data_const.kap;
...
void Create_Block_of_FluidWP(Particle_system  &particles, double x_0, double y_0, double x_1, double y_1)
{
	...
	for(int i = 0; i < npart_x+1; i ++){
		...
		for(int j = 0; j < npart_y+1; j++){
			...
			real rhoin = Pressure_to_density(9.81*(y_1 - r.y)*1000, 8960., particles.data_const.cs); ///to původní
			//real rhoin =8960; //o patro váš pro vodu toto pro solid snad

			//std::cout << "TRALALA: " << rhoin << std::endl;
			particles.Add_particle(fluid, 9.81*(y_1 - r.y)*1000, rhoin, r, v, a, 1);
...
void Create_solid(Particle_system  &particles, double x_0, double y_0, double x_1, double y_1)
{
	...
	for(int i = 0; i < npart_x+1; i ++){
		...
		for(int j = 0; j < npart_y+1; j++){
			...
			//std::cout << "TRALALA: " << rhoin << std::endl;
			particles.Add_particle(solid, 9.81*(y_1 - r.y)*1000, rhoin, r, v, a, 0);
