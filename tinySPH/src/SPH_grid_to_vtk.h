...
(int ncx, int ncy, double kh, double x_0, double y_0, std::vector<realvec> v, std::vector<real> rho, std::vector<real> press, std::vector<real> tem, std::string filename)
{
...
/*
	...
	*/
	...
	file << "TEMPERATURE 1 " << pocBunek << " float" << std::endl;
                for(int j=0;j<pocetBoduSiteY;j++){
        for(int i=0;i<pocetBoduSiteX;i++){
                        //      file << data[i*pocetBoduSiteY +j] << std::endl; //<------------------ TADY, pozor na indexování
                        file <<tem[POS(i,j, pocetBoduSiteX, pocetBoduSiteY)] << std::endl;
        }}
