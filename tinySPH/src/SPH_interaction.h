...
(Particle_system &particles)
{
	...
	for(int c = 0; c < particles.cells.size(); c++)
	{
		...
		real  dT_sum=0;//temperature term sumation
		...
		const real cpp=particles.data_const.cpp;
		double *kernel;
		...
		for(int i = 0; i < particles.cells[zz].cp.size(); i++)
		{
			...
			for(int &cl: ac)
			{
				...
				// ===========TEMPERATURE EQN ========//
				//c_pa*dT=sum_b(m_b/rhoa*rhob)(kappa_a +kappa_b)(T_a-T_b)
				//dT_sum +=fce(aT,nT,arho,nrho)
				dT_sum += (1/cpp)*(aT-nT)*(m/(arho*nrho))*(4*kappa)*(drdW/drs*drs); 	
			     //   if(dT_sum !=0){std::cout<<"dt " <<dT_sum<<std::endl;}
			     //
			//	if(particles.data.part_type[ni] == wall){std::cout << "HEUREEEEEEEEEEEEEEEEEEKA" << std::endl;}
			...
			realvec zero={0.,0};
			particles.data.a[ai] = zero;// o patro víš původní
			//particles.data.rho[ai] = drho_sum*arho + diff_sum;
			//particles.data.drho[ai] = drho_sum + diff_sum;
			particles.data.drho[ai] =0;//o patro víš původní
				
			particles.data.dT[ai]=dT_sum;
		
		//	particles.data.dT[ai]=1000;
			if(particles.data.part_type[ai] == solid){
